...
listofpackages <- c("readr","tidyverse","lubridate",
                    "pracma", "rstudioapi","stringr",
                    "plotly", "ggplot2", "readxl", "data.table", 
                    "purrr", "naniar", "roll","bit64",
                    "geosphere", "scales", "tibble", 
                    "leaflet.extras", "leaflet", "shiny")
...
CALCULO_DISTANCIA <- function(X) {
  return(distm(matrix(data = as.numeric(c(X[3],X[2])), ncol = 2),
...
raw_data <- raw_data %>% 
  ...
  group_by(Ficha) %>% 
  mutate(diff_tiempo = c(0, diff(as.numeric(FechaLocal))),
...
consolidado <- raw_data %>% 
  ...
  summarise(Nivel = mean(litros_corregidos, na.rm = TRUE),
...
consolidado <- consolidado %>% 
  arrange(Ficha, Fecha, Hora, Intervalo_2) %>% 
  group_by(Ficha, Fecha, Hora, Intervalo_2) %>% 
  mutate(Delta = append(0,diff(Nivel)),
...
consolidado <-  consolidado %>% 
  arrange(Ficha, Fecha, Hora, Intervalo_2) %>% 
  group_by(Ficha) %>% 
  mutate(Delta = append(0, diff(litros_filtro)),
...
# DE UNA VARIABLE CUYO MAXIMO 100 (%) A 300 (LTS)
  ...
  vector_suma_delta <- append(vector_suma_delta, 
...
# SE DESESTIMAN AUMENTOS DE NIVEL FUERA DE LOS TALLERES.
  ...
  if (!is.null(picos)) {
    ...
    for (j in c(1:cant_cargas)) {
      ...
      chequeo_taller <- as.numeric(DISTANCIA_TALLERES(df$LAT[indices_cargas[j-z]],
                                                      df$LONG[indices_cargas[j-z]])[1])
...
# SE REGISTRA EL TALLER DONDE SE REALIZO LA CARGA
        taller_carga <- append(taller_carga,
                               DISTANCIA_TALLERES(df$LAT[indices_cargas[i]],
                                                  df$LONG[indices_cargas[i]])[2])
        ...
        if (i == 1){
          recorrido <- append(recorrido, 
                              sum(df$Distancia[df$Fecha_Hora < df$Fecha_Hora[indices_cargas[i]]],
                                  na.rm = TRUE) / 1000)
          lts_consumidos <- append(lts_consumidos, 
                                   sum(df$Delta[df$Fecha_Hora < df$Fecha_Hora[indices_cargas[i]]],
                                       na.rm = TRUE) * -3 + picos[i,1] * 3)
        } else {
          recorrido <- append(recorrido, 
                              sum(df$Distancia[df$Fecha_Hora < df$Fecha_Hora[indices_cargas[i]] &
...
listado_cargas <- data.frame(ficha_carga, taller, fecha_carga, fecha_hora_carga, cargas,
                             recorrido, lts_consumidos)

# SE REGISTRAN LAS BAJAS ABRUPTAS DE COMBUSTIBLE EN DATAFRAMES
posibles_perdidas <- data.frame(ficha_perdida, fecha_perdida, fecha_hora_perdida, 
                                cant_perdida, Lat_perdida, Long_perdida)

colnames(posibles_perdidas) <- c("Ficha", "Fecha", "Fecha_Hora", "Litros", "LAT", "LONG")
colnames(listado_cargas) <- c("Ficha", "Taller", "Fecha", "Fecha_Hora", "Litros_cargados", 
...
if (nrow(posibles_perdidas) > 0) {
  ...
  for (i in c(1:nrow(posibles_perdidas))) {
    ...
    analizar <- consolidado %>% 
      dplyr::filter(Ficha == posibles_perdidas$Ficha[i],
                    Fecha_Hora > inicio,
                    Fecha_Hora < fin) %>% 
...
datos_diarios_km <- consolidado %>% 
  ...
  summarise(KM = sum(Distancia, na.rm = TRUE)/1000,
...
datos_diarios_lts <- listado_cargas %>% 
  group_by(Ficha, Fecha) %>% 
  summarise(Taller_primera_carga = first(Taller),
...
datos_diarios <- datos_diarios %>% 
  mutate(Litros_carga = coalesce(Litros_carga, 0),
...
datos_horario <- consolidado %>% 
  ...
  summarise(Litros_consumidos = - 3 * sum(Delta, na.rm = TRUE),
            Distancia_recorrida = sum(Distancia, na.rm = TRUE) / 1000,
            LAT = last(LAT),
            LONG = last(LONG))

datos_horario <- datos_horario %>% 
  mutate(Consumo_medio = Litros_consumidos / Distancia_recorrida * 100,
...
Velocidad <- raw_data %>% 
  ...
  group_by(Ficha, Fecha, Linea) %>% 
  summarise(Metros = sum(Distancia, na.rm = TRUE),
            Segundos = sum(diff_tiempo, na.rm = TRUE),
            Velocidad = Metros / Segundos *3.6)

fichas_seleccion <- list(labels = vector_fichas,
