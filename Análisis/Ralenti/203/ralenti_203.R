...
ralenti_203 <- read_xlsx("ralenti_203.xlsx")
setDT(ralenti_203)
...
promedio_ralenti_fin <- round(mean(tail(dia_ralenti_203$Horas_ralenti, 7)))
ahorro <- promedio_ralenti_ini - promedio_ralenti_fin
disminucion <- round((1-promedio_ralenti_fin/promedio_ralenti_ini)*100)

label_ini <- paste0("Ralenti", "\n",
                    promedio_ralenti_ini, " hs diarias")
label_fin <- paste0("Ralenti", "\n",
                    promedio_ralenti_fin, " hs diarias")
label_placa <- paste0("Comienzo instalación", "\n",
                      "de corte automático")
label_ahorro <- paste0("Disminución de ", disminucion, "% de ralenti", "\n",
                       "Ahorro de ", ahorro, " hs diarias", "\n",
                       "Aproximadamente ",ahorro*30*2, " lts mensuales")

pos_texto = mean(dia_ralenti_203$Fecha)


ggplot()+
  geom_line(data = dia_ralenti_203,
  ...
  geom_smooth(data = dia_ralenti_203,
              aes(x = Fecha, y = Horas_ralenti),
              span = 1, se= FALSE, color = "purple", 
              alpha = 0.8, linetype = "longdash") +
  
  geom_segment(aes(x = as.Date("2023/06/01"), xend =  as.Date("2023/06/01"),
                   y = 250, yend = max(dia_ralenti_203$Horas_ralenti) + 50), 
               color = "black", linetype = "longdash", 
               linewidth = 1, alpha = 0.7) +
  
  geom_vline(xintercept = as.numeric(min(dia_ralenti_203$Fecha)) + 14,
             linetype = "longdash", color = "red", alpha = 0.6, linewidth = 1) +
  
  geom_vline(xintercept = as.numeric(max(dia_ralenti_203$Fecha)) - 7,
             linetype = "longdash", color = "red", alpha = 0.6, linewidth = 1) +
  ...
  geom_text(aes(x = max(dia_ralenti_203$Fecha) - 1, y = 50), 
            label = label_fin, 
            hjust = 0.5, 
            position = position_dodge(0.5), size = 6, 
            fontface = "bold") +
  ...
  geom_segment(aes(x = min(dia_ralenti_203$Fecha) + days(15), 
                   xend =  max(dia_ralenti_203$Fecha) - days(8),
                   y = 50, yend = 50), 
               color = "blue", linewidth = 3, 
               arrow = arrow(), alpha = 0.8) +
  
  geom_segment(aes(x = as.Date("2023/06/01"), 
  ...
  labs(title = "Horas de Ralenti 203",
       subtitle = paste0("Talleres de Saavedra, Moreno y Pilar (período: ", 
